...
angular.module('angular.directives-round-progress', []).directive('angRoundProgress', [function () {
	var compilationFunction = function (templateElement, templateAttributes, transclude) {
		if (templateElement.length === 1) {
			...
			var outerShadowBlur = node.getAttribute('data-round-progress-outer-shadow-blur') || '10';
			var outerCircleWidth = node.getAttribute('data-round-progress-outer-circle-width') || '20';
			var innerCircleWidth = node.getAttribute('data-round-progress-inner-circle-width') || '5';

			var outerShadowColor = node.getAttribute('data-round-progress-outer-shadow-color') || '#000';
				...
				pre: function preLink(scope, instanceElement, instanceAttributes, controller) {
					...
					scope.$watch(expression, function (newValue, oldValue) {
						...
						*/
						...
						ctx.beginPath();
						ctx.shadowColor= outerShadowColor;
						ctx.shadowBlur = parseInt(outerShadowBlur);
						ctx.shadowColor= outerShadowColor;
						ctx.shadowBlur = parseInt(outerShadowBlur);
